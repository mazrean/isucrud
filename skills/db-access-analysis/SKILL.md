...
解析対象の Go プロジェクトのルート（`go.mod` があるディレクトリ）に移動した上で、以下のいずれかの形式で実行する。

#### A. Markdown を生成（エージェントによる解析向け・推奨）

```sh
isucrud -dst ./dbdoc.md ./...
```

- `./dbdoc.md` に Mermaid 形式の DB アクセスグラフが書き出される。
- エージェントはこのファイルを `Read` ツールで読むことで、関数とテーブルの依存関係を把握できる。

#### B. Web UI で確認したい場合（人間が探索する場合）

```sh
isucrud -web -addr :7070 ./...
```

ブラウザで `http://localhost:7070` を開く。エージェント単体での自動解析には不向き。
...
生成される `dbdoc.md` には Mermaid グラフが含まれており、ノードとエッジが次の意味を持つ。
...
ループ内で発行されているクエリは、isucrud 内部では `InLoop` フラグが立っており、Web UI ではエッジ表示で区別される。Markdown でも対応するエッジに印が付くため、N+1 の候補として特定できる。

### 5. エージェントによる典型的な解析パターン

`dbdoc.md` を生成したあと、エージェントは以下のような解析を行う:

1. **テーブル単位の利用箇所抽出**
   - `dbdoc.md` 内で `--> table:<対象テーブル>` を含む行を grep し、そのテーブルにアクセスしている関数を列挙する。
...
3. **N+1 候補の検出**
   - ループ内エッジ（Web UI 表示で破線になるもの／Mermaid 上で別系統の `linkStyle` が付くもの）を起点に、ループの呼び出し元を追って真の N+1 を特定する。
...
# 4. 結果を Read ツールで読み込む
#    (このスキルを呼んだエージェントが Read /path/to/dbdoc.md する)
```

実行後は `dbdoc.md` の Mermaid セクションを読み、`func:...` と `table:...` の対応関係をもとに必要な情報を抽出する。
